...
// Verifique se o efeito de som está sendo carregado corretamente
shootEffect = Mix_LoadWAV("assets/sounds/shoot.wav");
if (!shootEffect) {
    std::cerr << "Failed to load shoot effect: " << Mix_GetError() << std::endl;
} else {
    std::cout << "Shoot effect loaded successfully." << std::endl;
    Mix_VolumeChunk(shootEffect, MIX_MAX_VOLUME); // Definindo o volume do efeito sonoro
}

    // enemyDestroyedEffect = Mix_LoadWAV("audio/enemy-fainted.wav");
    // if (!enemyDestroyedEffect) {
    //     std::cerr << "Failed to load enemy destroyed effect: " << Mix_GetError() << std::endl;
    // }
    ...
    while (eventInterface->getIsRunning()) {
        ...
        update((SDL_GetTicks() - lastUpdateTick) / 1000.0f);
